...
export async function searchColorsAction(query: string): Promise<ActionResult<Color[]>> {
  try {
    ...
    const results = await db
      .select()
      .from(colors)
      .where(like(colors.name, searchQuery))
      .orderBy(desc(colors.createdAt));
...
export async function createColorAction(input: ColorFormData): Promise<ActionResult<Color>> {
  try {
    // Require admin role to create colors
    const session = await requireAuth({ roles: ['admin'] });
    logger.info({ colorName: input.name, hex: input.hex, userId: session.user.id, userEmail: session.user.email }, 'Đang tạo màu');
    ...
    const [existing] = await db
      .select()
      .from(colors)
      .where(eq(colors.name, data.name))
      .limit(1);

    if (existing) {
      logger.warn({ colorName: data.name }, 'Màu đã tồn tại');
      return {
        success: false,
    ...
    logger.info({ colorId, colorName: data.name, hex: data.hex }, 'Đang thêm màu vào cơ sở dữ liệu');
    ...
    logger.info({ colorId, colorName: data.name }, 'Tạo màu thành công');
    ...
    if (error instanceof Error && error.message.includes('quyền')) {
      logger.warn({ error: error.message, colorName: input.name }, 'Không có quyền tạo màu');
      return {
        success: false,
        error: error.message,
      };
